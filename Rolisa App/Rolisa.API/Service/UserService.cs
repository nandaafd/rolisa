...
namespace Rolisa.API.Service
{
    public class UserService
    {
        ...
        public UserService(RolisaContext _db) { db = _db; }
        public VMResponse GetAll()
        {
            List<User> users = new List<User>();
            users = db.Users.Where(u => u.IsDeleted ==false).ToList();
            if (users != null)
            {
                response.statusCode = System.Net.HttpStatusCode.OK;
                response.message = "success fetch data user";
                response.data = users;
            }
            else
            {
                response.statusCode = System.Net.HttpStatusCode.NoContent;
                response.message = "user has no data";
                response.data = null;
            }
            return response;
        }
        ...
        {
            User? user = db.Users.Where(u => u.Email == email && u.IsDeleted == false && u.IsBlocked == false).FirstOrDefault();
            response.statusCode = System.Net.HttpStatusCode.OK;
            response.message = "success get data " + email;
            response.data = user;
            return response;
        }
        public List<VMResetPassword> GetByEmailForResetPass(string email)
        {
        ...
        public VMResponse ChangePassword(VMUser data)
        {
            using (IDbContextTransaction dbTran = db.Database.BeginTransaction())
            {
                try
                {
                    ...
                    List<VMResetPassword> passwordHistory = GetByEmailForResetPass(data.Email);
